...
namespace tHerdBackend.Core.Interfaces.Products
{
    public interface IProdProductRepository
    {
        ...
        Task<(IEnumerable<ProdProductSearchDto> list, int totalCount)> GetAllFrontAsync(ProductFrontFilterQueryDto query, CancellationToken ct = default);
        Task<ProdProductDetailDto?> GetByIdAsync(int id, int? skuId, CancellationToken ct = default);
