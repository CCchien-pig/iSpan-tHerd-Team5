              ...
              <div class="row align-items-center">
                <div class="col-md-7">
                  ...
                  <div class="mb-3 fs-4">
                    ...
                    <span>總金額: </span>
                    <strong class="main-color-green-text"
                      >NT$ {{ order.totalAmount.toLocaleString() }}</strong
                    >
                  </div>
                ...
                <div class="col-md-5">
                  <div class="d-flex flex-column gap-2 align-items-stretch">
                    ...
                    <div
                      v-if="order.canReturn && !order.hasRmaRequest"
                      class="badge orange-badge py-2 fs-6"
                    >
                      <i class="bi bi-check-circle me-1"></i>可申請退換貨 ({{
                        getRemainingDays(order.deliveredDate)
                      }}天內)
                    </div>
                  </div>
                </div>
              ...
              <div class="bg-light p-4 rounded">
                <p class="mb-3 fs-5">
                  ...
                  <span class="ms-2 text-muted"
                    >({{ rma.refundScope === 'order' ? '整筆訂單' : '部分商品' }})</span
                  >
                </p>
      ...
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          <div class="modal-header main-color-green">
            <h3 class="modal-title main-color-white-text">
              <i class="bi bi-receipt me-2"></i>訂單詳情
            </h3>
            <button
              type="button"
          ...
          <div class="modal-body p-4">
            <div v-if="orderDetail">
              ...
              <div class="mb-4 p-4 bg-light rounded">
                <h4 class="mb-3 main-color-green-text">
                  <i class="bi bi-info-circle me-2"></i>訂單狀態
                </h4>
                <div class="row">
                  <div class="col-md-4 mb-3">
                    <p class="mb-2 fs-5"><strong>付款狀態:</strong></p>
                    <span
                      :class="getPaymentStatusClass(orderDetail.order.paymentStatus)"
                      class="fs-5 px-3 py-2"
                    >
                  ...
                  <div class="col-md-4 mb-3">
                    ...
                    <span
                      :class="getShippingStatusClass(orderDetail.order.shippingStatusId)"
                      class="fs-5 px-3 py-2"
                    >
                  ...
                  <div class="col-md-4 mb-3">
                    ...
                    <span
                      :class="getOrderStatusClass(orderDetail.order.orderStatusId)"
                      class="fs-5 px-3 py-2"
                    >
              ...
              <!-- 收件資訊 -->
              <div class="mb-4">
                <h4 class="border-bottom pb-3 mb-3 main-color-green-text">
                  <i class="bi bi-person-circle me-2"></i>配送資訊
                </h4>
                <div class="row fs-5 g-3">
                  <div class="col-12">
                    ...
                    <span
                      ...
                      :class="
                        orderDetail.order.deliveryMethod === 'CVS' ? 'bg-success' : 'bg-primary'
                      "
                    >
                      {{ orderDetail.order.deliveryMethod === 'CVS' ? '超商取貨' : '宅配到府' }}
                      <span v-if="orderDetail.order.logisticsSubType">
                        ({{ getLogisticsSubTypeText(orderDetail.order.logisticsSubType) }})
                      </span>
                    </span>
                  </div>

                  <div class="col-md-6">
                    <strong>收件人:</strong> {{ orderDetail.order.receiverName }}
                  </div>
                  <div class="col-md-6">
                    <strong>電話:</strong> {{ orderDetail.order.receiverPhone }}
                  </div>

                  <template v-if="orderDetail.order.deliveryMethod === 'CVS'">
                    <div class="col-md-6" v-if="orderDetail.order.cvsStoreName">
                      <strong>取貨門市:</strong> {{ orderDetail.order.cvsStoreName }}
                    </div>
                    <div class="col-md-6" v-if="orderDetail.order.cvsStoreID">
                      <strong>門市代號:</strong> {{ orderDetail.order.cvsStoreID }}
                    </div>
                    <div class="col-12">
                      <strong>門市地址:</strong> {{ orderDetail.order.receiverAddress }}
                    </div>
                  </template>

                  <template v-else>
                    <div class="col-12">
                      <strong>收件地址:</strong> {{ orderDetail.order.receiverAddress }}
                    </div>
                  </template>

                  <div v-if="orderDetail.order.trackingNumber" class="col-12 mt-3 pt-3 border-top">
                    <strong>物流單號:</strong>
                    <span class="font-monospace ms-2">{{ orderDetail.order.trackingNumber }}</span>
                  </div>
                </div>
              </div>
                    ...
                    <tbody>
                      <tr v-for="item in orderDetail.items" :key="item.orderItemId">
                        ...
                        <td>
                          <span
                            class="badge"
                            style="background-color: rgb(77, 180, 193); font-size: 1rem"
                            >{{ item.specCode }}</span
                          >
                        </td>
              ...
              <div class="p-4 rounded" style="background-color: #e8f4f8">
                ...
                <div class="d-flex justify-content-between mb-3 fs-4">
                  ...
                  <span class="text-danger"
                    >-NT$ {{ orderDetail.order.discountTotal.toLocaleString() }}</span
                  >
                </div>
                ...
                <div class="d-flex justify-content-between border-top pt-3">
                  ...
                  <h3 class="mb-0 fw-bold main-color-green-text">
                    NT$ {{ orderDetail.order.totalAmount.toLocaleString() }}
                  </h3>
                </div>
              </div>
            </div>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <div class="modal-body p-4">
            <div class="mb-4">
              <label class="form-label fs-4 fw-bold"
                ><i class="bi bi-list-ul me-2"></i>申請類型</label
              >
            ...
            <div class="mb-4">
              <label class="form-label fs-4 fw-bold"
                ><i class="bi bi-diagram-3 me-2"></i>退款範圍</label
              >
            ...
            <div class="mb-4">
              <label class="form-label fs-4 fw-bold"
                ><i class="bi bi-pencil-square me-2"></i>詳細說明</label
              >
              <textarea
                class="form-control form-control-lg fs-5"
                v-model="rmaForm.reason"
                rows="4"
                placeholder="請詳細描述退換貨原因..."
              ></textarea>
            </div>
            <div class="mb-4">
              <label class="form-label fs-4 fw-bold"
                ><i class="bi bi-image me-2"></i>上傳照片</label
              >
          ...
          <div class="modal-footer">
            <button
              type="button"
              class="btn silver-reflect-button fs-5 px-5"
              data-bs-dismiss="modal"
            >
              取消
            </button>
...
export default {
  ...
  methods: {
    ...
    // 在 methods 中新增
    getLogisticsSubTypeText(subType) {
      const map = {
        UNIMARTC2C: '7-ELEVEN',
        FAMIC2C: '全家',
        HILIFEC2C: '萊爾富',
